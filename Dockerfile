...
RUN apt-get -qq update && \ 
    ...
    python-is-python3 java-8-openjdk \
...
###################################################
RUN mkdir -p /etc/java-8-openjdk/security && echo "jdk.tls.disabledAlgorithms=SSLv3, RC4, DES, MD5withRSA, DH keySize < 1024, EC keySize < 224, 3DES_EDE_CBC, anon, NULL, include jdk.disabled.namedCurves" | tee -a /etc/java-8-openjdk/security/java.security
